...
public static class SemaphoreHolder
{
    ...
    public static int best_exp_count_tolerance = 2;
...
class Meter
{
    ...
    public void startMeterThread()
    {
        if (errorCode == 0)
        {
            ...
            inxR = ocean.getIndicesAtWavelengthRange(DeviceID, ref errorCode, ref waves, lo, hi);
...
class Camera
{
    ...
    //保存图片名
    private static string datetime = Convert.ToString(DateTime.Now.ToString("yyyy-MM-dd-HH_mm"));
    ...
    public void run()
    {
        ...
        while (true)
        {
            ...
            if (SemaphoreHolder.is_changed)
            {
                ...
                ASICameraDll2.ASISetROIFormat(ICameraID, m_iCurWidth, m_iCurHeight, 0, cur_type);
        ...
        else if (cur_type == ASICameraDll2.ASI_IMG_TYPE.ASI_IMG_RAW16)
        {
            ...
            for (int i = 1; i < 256; i++)
            {
                Cv2.Line(
                    ...
                    new OpenCvSharp.Point(
                        bin_w * i,
                        histogram_height - offset_y - (int)Math.Round(sampledHist.At<float>(0, i))
                    ),
                    new Scalar(0, 0, 0),
                    2,
                    LineTypes.Link8,
